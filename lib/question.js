 ...
 */
class RegradParsingError extends Error {
  constructor(message) {
    super(message);
    this.name = 'RegradParsingError';
  }
}
        ...
        async.series([
            ...
            (callback) => {
                if (!data.gradable) {
                    ...
                    callback(new RegradParsingError());
                    return;
                }
            ...
            // catch RegradParsingError as we are just using it to exit with no action
            if (err instanceof RegradParsingError) {
